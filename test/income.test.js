...
describe('GET /api/incomes/:incomeId', function () {
  beforeEach(async () => {
      await createTestUser();
      await createTestIncome();
  })
  ...
  it('should can get contact', async () => {
      const testIncome = await getTestIncome();

      const result = await supertest(web)
          .get("/api/incomes/" + testIncome.id)
          .set('Authorization', 'test');
      expect(result.status).toBe(200);
  ...
  it('should return 404 if contact id is not found', async () => {
      ...
      const result = await supertest(web)
          .get("/api/incomes/" + (testIncome.id + 1))
          .set('Authorization', 'test');
...
describe('PUT /api/incomes/:incomeId', function () {
  beforeEach(async () => {
      await createTestUser();
      await createTestIncome();
  })
  ...
  it('should reject if request is invalid', async () => {
      const testContact = await getTestIncome();

      const result = await supertest(web)
          .put('/api/incomes/' + testContact.id)
          .set('Authorization', 'test')
          .send({
            category: "",
            description: "",
...
//       const result = await supertest(web)
//           .put('/api/incomes/' + (testContact.id + 1))
//           .set('Authorization', 'test')
//           .send({
//               first_name: "Eko",
//               last_name: "Khannedy",
...
describe('DELETE /api/contacts/:contactId', function () {
  ...
  it('should can delete Income', async () => {
      let testIncome = await getTestIncome();
      const result = await supertest(web)
          .delete('/api/incomes/' + testIncome.id)
          .set('Authorization', 'test');

      expect(result.status).toBe(200);
      expect(result.body.data).toBe("OK");

      testIncome = await getTestIncome();
  ...
  it('should reject if Income is not found', async () => {
      let testIncome = await getTestIncome();
      const result = await supertest(web)
          .delete('/api/incomes/' + (testIncome.id + 1))
          .set('Authorization', 'test');
